...
function autoPlay() {
    ...
    //Round 8
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(35*1000-1).then(executeUpdate)
    wait(35*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(36*1000-1).then(executeUpdate)
    wait(36*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(37*1000-1).then(executeUpdate)
    wait(37*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(38*1000-1).then(executeUpdate)
    wait(38*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(39*1000-1).then(executeBidWinnerCheck)
    wait(39*1000).then(executeUpdate)

    //Round 9
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(40*1000-1).then(executeUpdate)
    wait(40*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(41*1000-1).then(executeUpdate)
    wait(41*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(42*1000-1).then(executeUpdate)
    wait(42*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(43*1000-1).then(executeUpdate)
    wait(43*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(44*1000-1).then(executeBidWinnerCheck)
    wait(44*1000).then(executeUpdate)

    //Round 10
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(45*1000-1).then(executeUpdate)
    wait(45*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(46*1000-1).then(executeUpdate)
    wait(46*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(47*1000-1).then(executeUpdate)
    wait(47*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(48*1000-1).then(executeUpdate)
    wait(48*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(49*1000-1).then(executeBidWinnerCheck)
    wait(49*1000).then(executeUpdate)

    //Round 11
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(50*1000-1).then(executeUpdate)
    wait(50*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(51*1000-1).then(executeUpdate)
    wait(51*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(52*1000-1).then(executeUpdate)
    wait(52*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(53*1000-1).then(executeUpdate)
    wait(53*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(54*1000-1).then(executeBidWinnerCheck)
    wait(54*1000).then(executeUpdate)

    //Round 12
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(55*1000-1).then(executeUpdate)
    wait(55*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(56*1000-1).then(executeUpdate)
    wait(56*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(57*1000-1).then(executeUpdate)
    wait(57*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(58*1000-1).then(executeUpdate)
    wait(58*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(59*1000-1).then(executeBidWinnerCheck)
    wait(59*1000).then(executeUpdate)

    //Round 13
    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(60*1000-1).then(executeUpdate)
    wait(60*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(61*1000-1).then(executeUpdate)
    wait(61*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(62*1000-1).then(executeUpdate)
    wait(62*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(63*1000-1).then(executeUpdate)
    wait(63*1000).then(executePlay)

    var wait = ms => new Promise((r, j)=>setTimeout(r, ms));
    wait(64*1000-1).then(executeBidWinnerCheck)
    wait(64*1000).then(executeUpdate)
...
function allocateBidWinner(){
    ...
    console.log(has_anyone_won(bidWinner, partnerPlayer, currentBid, setWins))
    if(has_anyone_won(bidWinner, partnerPlayer, currentBid, setWins) instanceof String){
        gameOver = true;
        winningPair = has_anyone_won(bidWinner, partnerPlayer, currentBid, setWins);
    };
    clearPlayedCards();
        ...
        if(cardPlayed[1] && cardPlayed[2] && cardPlayed[3] && cardPlayed[4]){ //If all cards played
            ...
            if(gameOver){ // If game is over, remove next player button and display who is the winner and who is the bidder's partner
                ...
                surface.append('text')
                    ...
                    .text(function(d) {return "The bidder's partner is " + partnerPlayer;});
                for(i = 1;i < 5; i++){
                    surface.select('.scoreBoard').select('.player' + i + 'score').text(function(d) {return 'Player ' + i + ' Sets Won: ' + setWins[i];});
                }   
            }
            currentPlayer = setWinner;
            initPlayer = setWinner;
