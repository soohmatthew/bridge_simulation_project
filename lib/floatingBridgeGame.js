...
function allocateBidWinner(){
    ...
    console.log(typeof has_anyone_won(bidWinner, partnerPlayer, currentBid, setWins) === "string");
...
var totalSetWins = {1:0,2:0,3:0,4:0};
var winnerHistory = {'bidder':0,'opponent':0};

function generateWinBoard(){
    surface.append('g').attr('class','winningBoard')
    surface.select('.winningBoard').append('text')
        .attr("x",function(d){return (maxCols*0.45)*cellWidth+"px";})
        .attr("y",function(d){return 10*0.5*cellHeight+"px";})
        .attr('font-size','20px')
        .attr('fill','white')
        .attr('font-weight',"bold")
        .text(function(d) {return 'Player Wins';});
    surface.select('.winningBoard').append('text')
        .attr('class','player' + i + 'score')
        .attr("x",function(d){return (maxCols*0.44)*cellWidth+"px";})
        .attr("y",function(d){return (12)*0.5*cellHeight+"px";})
        .attr('font-size','20px')
        .attr('text-align','center')
        .attr('fill','white')
        .attr('font-weight',"bold")
        .text(function(d) {return 'Bidder Wins: ' + winnerHistory.bidder;});
    surface.select('.winningBoard').append('text')
        .attr('class','player' + i + 'score')
        ...
        .attr('fill','white')
        .attr('font-weight',"bold")
        .text(function(d) {return 'Opponent Wins: ' + winnerHistory.opponent;});
}
...
function autoPlayNA(){
    ...
    for(i = 1; i < 5; i++){
        totalSetWins[i] = Number(totalSetWins[i])/n
    }
    console.log(totalSetWins);
    console.log(winnerHistory);
    for(i = 1;i < 5; i++){
        surface.select('.scoreBoard').select('.player' + i + 'score').text(function(d) {return 'Player ' + i + ' Average Sets Won: ' + totalSetWins[i];});
    }
    generateWinBoard()
}
    ...
    while(!gameOver){
        if(cardPlayed[1] && cardPlayed[2] && cardPlayed[3] && cardPlayed[4]){ //If all cards played
            ...
            if(gameOver){ // If game is over, remove next player button and display who is the winner and who is the bidder's partner
                ...
                for(i = 1; i < 5; i++){
                    totalSetWins[i] += setWins[i];
                    setWins[i] = 0;
                }
                winnerHistory[winningPair] = winnerHistory[winningPair] + 1
                /*if(winningPair == 'bidder'){
                    winnerHistory[bidWinner] = winnerHistory[bidWinner] + 1
                    winnerHistory[partnerPlayer] = winnerHistory[partnerPlayer] + 1
                } else {
                    for(i = 1; i < 5; i++){
                        if(i != bidWinner && i!= partnerPlayer){winnerHistory[i] = winnerHistory[i] + 1}
                    }
                }*/
            }
            currentPlayer = setWinner;
