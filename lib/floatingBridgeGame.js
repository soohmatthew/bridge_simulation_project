...
function createPlayerIndicator(currentPlayer){
    ...
    if(startGame && !gameOver){
        ...
        if(cardPlayed[currentPlayer] || !partnerChosen) {
            ...
            if(cardPlayed[1] && cardPlayed[2] && cardPlayed[3] && cardPlayed[4] && partnerChosen){
                setWinner = determine_which_player_won_round(cardPile); //Find a way to incorporate trumpBroken inside this
                setWins[setWinner] += 1;
                ...
                if(!isNaN(has_anyone_won_the_game(setWins,partnerChosen,bidWinner))){
                    gameOver = true;
                    winningPair = has_anyone_won_the_game(setWins,partnerChosen,bidWinner);
                }
                clearPlayedCards();
...
function submitPartnerChoice(){
    ...
    if(check_partner_validity({'suite':info[0].value,'number':info[1].value})[1]){ //Matthew's function
        partnerChosen = true;
        document.getElementById("partnerFormContainer").style.display = "none";
        partnerPlayer = check_partner_validity({'suite':info[0].value,'number':info[1].value})[0];
    } else {
...
function playCard(suite, number, ID){
    if (!cardPlayed[currentPlayer] && partnerChosen) {
        if (confirm('You are playing the ' + cardNames[number] + ' of ' + suite)) {
            if(check_if_able_to_play_card(initPlayer,cards[currentPlayer],{'suite':suite,'number':number})){ // Matthew's function for check if able to play card
