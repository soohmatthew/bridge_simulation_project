...
3:{"row":8,"col":maxCols/2-4.9},
4:{"row":8,"col":maxCols/2-2}
};
...
//import { distribute_cards_checked } from 'Backend Logic/shuffle_distribute.js';
...
function distributeCards(){
    for(let i = 0; i < 13; i++){
        ...
        cards[3][i]['location'] = {'row':21.5,'col':7+(i+1)};
        cards[4][i]['location'] = {'row':2,'col':5+2*(i+1)};
    }
    console.log("Distributing Cards");
    d3.select('.player1Cards').selectAll('image').data(cards[1]).transition()
        .duration(2000)
        .attr("x",function(d){var cell= getLocationCell(d.location); console.log(cell.x); return cell.x+"px";})
        .attr("y",function(d){var cell= getLocationCell(d.location); console.log(cell.y); return cell.y+"px";})
