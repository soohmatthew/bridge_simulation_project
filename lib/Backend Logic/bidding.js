...
function check_partner_validity(bid_winner, cards, chosen_parter_number, chosen_partner_suite){
    ...
    function card_in_own_deck(card){return card.number === chosen_parter_number && card.suite === chosen_partner_suite}
    // If checks if card is in own deck
    if (bid_winner_cards.filter(card_in_own_deck).length > 0){
        return ([null, false])
    }
    else {
        for (let partner = 0; partner < possible_partners.length; partner++) {
            let possible_partner_card = cards[possible_partners[partner]]
            if (possible_partner_card.filter(card_in_own_deck).length > 0){return ([possible_partners[partner], true])}
        }}
}
...
function check_if_able_to_play_card(current_player, starting_player, currentBid, cardPile, card_chosen, cards){
    ...
    //      If he does not:
    //          he can throw any card
    //          if card is trump suite:
    //              trump_broken = true
    let trump = currentBid.suite
    let players = [1,2,3,4]
    ...
    if (current_player === starting_player){
        //     if trump is broken
        if (trump_broken){return true} //         he can play whatever card he wants
...
//console.log(check_partner_validity(bid_winner, cards, "4", "Hearts"))
