...
function distribute_cards_checked(initial_location){
    ...
    function filterByClubs(item){return item.weight === "Clubs"}
    function filterByDiamonds(item){return item.weight === "Diamonds"}
    function filterByHearts(item){return item.weight === "Hearts"}
    function filterBySpades(item){return item.weight === "Spades"}

    const sort_asc = (a,b) => {return parseInt(a.cardType) - parseInt(b.cardType)};
